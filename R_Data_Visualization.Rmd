...
```{r}
ggplot(data.long, aes(x=treatment, y=score, group=interaction(control, treatment), colour=control)) + 
  geom_point   (size=1)  +
  geom_line    (linetype="dashed" ,aes(group=interaction(subj,control), alpha=.5)) +
  geom_line    ( stat = "summary", fun.y = "mean", size=1.5 , aes(group=control))  +
  geom_point   ( stat = "summary", fun.y = "mean", size=2 , aes(group=control)) +
  geom_errorbar( stat = "summary", fun.data = "mean_se", fun.args = 2, width = 0.3, size=1 , aes(group=control))
```
